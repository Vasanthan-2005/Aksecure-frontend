      ...
      <div className="flex-1 overflow-y-auto bg-slate-950 relative custom-scrollbar">
        ...
        <div className="p-3 md:p-4 space-y-2 md:space-y-4 relative z-10 w-full max-w-full">
          ...
          <div className="mt-4">
            ...
            {(serviceRequest.cameraType || serviceRequest.cameraCount) && (
              <div className="bg-slate-800/40 rounded-xl p-4 md:p-6 border border-white/5 mt-4">
                <h4 className="text-[11px] md:text-sm font-bold text-slate-500 uppercase tracking-tight mb-3">
                  {serviceRequest.category === 'CCTV' ? 'Camera' : 'Device'} Details
                </h4>
                <div className="grid grid-cols-2 gap-4">
                  {serviceRequest.cameraType && (
                    <div>
                      <span className="text-xs text-slate-400 block mb-1">Type</span>
                      <span className="text-white font-medium">{serviceRequest.cameraType}</span>
                    </div>
                  )}
                  {serviceRequest.cameraCount && (
                    <div>
                      <span className="text-xs text-slate-400 block mb-1">Quantity</span>
                      <span className="text-white font-medium">{serviceRequest.cameraCount}</span>
                    </div>
                  )}
                </div>
              </div>
