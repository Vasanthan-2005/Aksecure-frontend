import { Clock, Image as ImageIcon, Tags } from 'lucide-react';
import { useState, useEffect } from 'react';
import PriceTableRenderer from '../../common/components/PriceTableRenderer';
    ...
    <>
      <div className="glass-card p-6 rounded-2xl border border-slate-700/50 shadow-xl bg-slate-900/60 backdrop-blur-xl">
        <div className="flex items-center gap-3 mb-6 pb-4 border-b border-white/5">
          ...
          <h3 className="text-[10px] font-black uppercase tracking-[0.2em] text-slate-500">Timeline Activity ({timeline.length})</h3>
        </div>
                ...
                <div className={`p-5 rounded-xl border hover:bg-slate-800/40 ${isAdmin
                  ...
                  <div className="flex items-start justify-between mb-3">
                    <div className="flex items-center gap-2">
                      <span
                        className={`font-black text-sm tracking-tight ${isAdmin ? 'text-white' : 'text-slate-200'
                          }`}
                      >
                        {item.addedBy}
                      </span>
                      {isAdmin && (
                        <span className="px-2 py-0.5 bg-violet-500/10 text-violet-400 border border-violet-500/20 rounded-md text-[9px] font-black uppercase tracking-widest">
                          Support
                        </span>
                      )}
                    </div>
                  ...
                  <p
                    className={`text-sm leading-relaxed mb-4 tracking-tight font-medium ${isAdmin ? 'text-slate-300' : 'text-slate-400'
                      }`}
                  >
                    {item.note}
                  </p>
                  ...
                  {images.length > 0 && (
                    <div className="mt-4 pt-4 border-t border-white/5">
                      <div className="flex items-center gap-2 mb-3">
                        ...
                        <span className="text-[10px] font-black text-slate-500 uppercase tracking-[0.2em]">Attached Images ({images.length})</span>
                      </div>
