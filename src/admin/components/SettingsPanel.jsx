    ...
    <div className="glass-card p-4 md:p-8 bg-slate-900/60 backdrop-blur-2xl border border-slate-700/50 rounded-3xl shadow-2xl w-full max-w-2xl max-h-[85vh] overflow-y-auto custom-scrollbar relative">
      <div className="mb-6 flex justify-between items-start gap-4 ">
        <div>
          ...
          <p className="text-xs md:text-slate-400 mt-2">
            Manage the contact information displayed to users in their portal.
          </p>
        </div>
        {onClose && (
          <button
      ...
      <form onSubmit={handleSave} className="space-y-4">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-6">
          ...
          <div className="bg-slate-900/40 border border-white/5 rounded-2xl p-3 md:p-4 hover:border-emerald-500/20 group md:col-span-2">
            ...
            <div className="flex flex-col md:flex-row gap-3 md:gap-4 items-stretch md:items-center">
              <input
                ...
                onChange={(e) => {
                  ...
                  setSettings({ ...settings, supportWhatsApp: digitsOnly });
                }}
            ...
            <p className="text-[9px] text-slate-500 mt-1 px-1 italic">
              Digits only, include country code (e.g. 91 for India).
            </p>
          </div>
        </div>
        ...
        {message.text && (
          ...
          >
            {message.type === "success" ? (
              ...
              <div className="w-5 h-5 rounded-full bg-red-500/20 flex items-center justify-center font-bold text-xs">
                !
              </div>
            )}
