...
import { MessageSquare, Clock, Check, Calendar, Laptop, IndianRupee, FileText, X, Download, ExternalLink } from 'lucide-react';
...
const AdminReplies = ({ refreshKey = 0 }) => {
  ...
  return (
    <div className="space-y-4">
            ...
            <div className="flex items-start gap-4 relative z-10">
              ...
              <div className="flex-1 min-w-0">
                  ...
                  <div className="flex-1">
                    <div className="flex flex-col sm:flex-row sm:items-center gap-2 mb-2">
                      <span className="font-bold text-white group-hover:text-blue-400 transition-colors truncate">
                        {(reply.addedBy && reply.addedBy.toLowerCase().includes('sivadass')) ? 'Admin Team' : (reply.addedBy || 'Support Specialist')}
                      </span>
                ...
                <div className="bg-slate-950/40 rounded-2xl p-5 border border-white/5 mb-4 group-hover:border-blue-500/10 transition-colors">
                  ...
                  {(reply.priceList?.length > 0 || reply.images?.length > 0) && (
                    <div className="mt-4 pt-4 border-t border-white/5">
                      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                        <div className="flex items-center gap-2">
                          <IndianRupee className="w-4 h-4 text-emerald-400" />
                          <span className="text-[10px] sm:text-xs font-bold text-slate-400 uppercase tracking-wider">
                            {reply.images?.length > 0 ? 'Bill / Quote Attached' : 'Quotation Available'}
                          </span>
                        </div>
                        <button
                          type="button"
                          onClick={() => {
                            if (reply.images?.length > 0) {
                              setSelectedAttachments(reply.images);
                            } else {
                              setSelectedQuote({ items: reply.priceList, total: reply.totalPrice });
                            }
                          }}
                        ...
                        >
                          {reply.images?.length > 0 ? <FileText className="w-3.5 h-3.5" /> : <IndianRupee className="w-3.5 h-3.5" />}
                          View Estimated Bill
                        </button>
                      </div>
                    </div>
      ...
      {/* Mobile Focused Image Modal */}
      {
        selectedAttachments && (
          <div className="fixed inset-0 z-[9999] flex items-center justify-center p-4 bg-black/90 backdrop-blur-md animate-in fade-in duration-200">
            <div className="bg-slate-900 w-full max-w-5xl max-h-[90vh] flex flex-col shadow-2xl border border-slate-700/50 rounded-2xl overflow-hidden relative">
              <div className="flex-shrink-0 bg-slate-950 border-b border-white/5 px-6 py-4 flex items-center justify-between z-10">
                <div>
                  <h3 className="text-xl font-bold text-white tracking-tight">Estimated Bill / Quote</h3>
                  <p className="text-sm text-slate-400">{selectedAttachments.length} file(s) attached</p>
                </div>
                <button
              ...
              <div className="flex-1 overflow-y-auto p-4 sm:p-6 custom-scrollbar bg-slate-900/50">
                <div className={`grid gap-6 ${selectedAttachments.length === 1 ? 'grid-cols-1' : 'grid-cols-1 sm:grid-cols-2'}`}>
                  {selectedAttachments.map((url, i) => {
                    const isPdf = url.toLowerCase().endsWith('.pdf');
                    return (
                      <div
                        key={i}
                        className="group relative bg-black/40 rounded-2xl overflow-hidden border border-white/5 hover:border-white/20 transition-all shadow-lg cursor-pointer sm:cursor-default"
                        onClick={() => {
                          if (!isPdf) {
                            setFocusedImage(url);
                          }
                      ...
                      >
                        {isPdf ? (
                          <div className="aspect-[4/3] flex flex-col items-center justify-center bg-slate-800/30 p-8 group-hover:bg-slate-800/50 transition-colors">
                            <div className="w-20 h-20 rounded-2xl bg-white/5 flex items-center justify-center mb-4 ring-1 ring-white/10 group-hover:scale-110 transition-transform duration-300">
                              <FileText className="w-10 h-10 text-slate-400 group-hover:text-blue-400 transition-colors" />
                            </div>
                            <span className="text-lg font-bold text-slate-200">PDF Document</span>
                            <span className="text-sm text-slate-500 mt-1">Click Open to view</span>
                          </div>
                        ) : (
                          <div className="w-full bg-black/50 flex align-center justify-center">
                            <img
                              src={url}
                              alt={`Bill Page ${i + 1}`}
                        ...
                        <div
                          className="hidden sm:flex absolute inset-0 bg-black/60 opacity-0 group-hover:opacity-100 transition-all duration-300 items-center justify-center gap-4 backdrop-blur-[2px]"
                        ...
                        >
                          <a
                            ...
                            className="inline-flex items-center gap-2 px-6 py-3 bg-blue-600 hover:bg-blue-500 text-white text-sm font-bold rounded-xl transition-all hover:scale-105 shadow-lg shadow-blue-600/20"
                            onClick={(e) => e.stopPropagation()}
                          >
                          ...
                          <a
                            href={url}
                            download
                            className="inline-flex items-center gap-2 px-6 py-3 bg-white/10 hover:bg-white/20 text-white text-sm font-bold rounded-xl transition-all hover:scale-105 border border-white/10"
                            onClick={(e) => e.stopPropagation()}
                          >
      ...
      {/* Focused Image Modal (Mobile) */}
      {
        focusedImage && (
          <div className="fixed inset-0 z-[10000] flex items-center justify-center bg-black animate-in fade-in duration-200">
            <button
              onClick={() => setFocusedImage(null)}
              className="absolute top-4 right-4 p-2 bg-white/10 text-white rounded-full backdrop-blur-md z-50 hover:bg-white/20 transition-colors"
            >
              <X className="w-6 h-6" />
            </button>

            <div className="w-full h-full flex items-center justify-center p-2" onClick={() => setFocusedImage(null)}>
              <img
                src={focusedImage}
                alt="Full Size View"
