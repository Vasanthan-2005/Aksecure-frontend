...
const ServiceRequestForm = ({ category, onSuccess, onCancel }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (formData.category === 'Fire Alarm' || formData.category === 'CCTV') {
        if (formData.cameraType) payload.append('cameraType', formData.cameraType);
        if (formData.cameraCount) payload.append('cameraCount', formData.cameraCount);
      }
          ...
          <div className="bg-slate-950/20 rounded-[28px] p-5 sm:p-8">
            ...
            <form onSubmit={handleSubmit} className="space-y-8">
              <div className="space-y-6">
                ...
                {/* Camera/Fire Alarm Details */}
                {(formData.category === 'Fire Alarm' || formData.category === 'CCTV') && (
                  <div className="mt-4 grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div>
                      <label className="block text-xs font-bold text-slate-500 uppercase tracking-widest mb-3 ml-1">
                        Type of {formData.category === 'CCTV' ? 'Camera' : 'Device'}
                      </label>
                      <input
                        type="text"
                        name="cameraType"
                        ...
                        onChange={handleChange}
                        placeholder={`E.g., ${formData.category === 'CCTV' ? 'Dome, Bullet, PTZ' : 'Smoke Detector, Heat Sensor'}`}
                        className="w-full compact-input bg-slate-900/50 border border-white/10 rounded-2xl text-white focus:outline-none focus:ring-2 focus:ring-violet-500/20 focus:border-violet-500/50 transition-all font-medium py-3 px-4"
                      />
                    </div>
                    <div>
                      <label className="block text-xs font-bold text-slate-500 uppercase tracking-widest mb-3 ml-1">
